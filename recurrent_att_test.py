...
try:
	opts, args = getopt(sys.argv[1:], "", ["batch_size=", "conv1_nr_filters=", "conv1_filter_size=", "conv1_stride=", "img_size=", "gru_dim=", "seq_len=", "use_cudnn", "zero_tail_fc", "var_len", "test", "acc_scale=",
		"zoom_scale=", "dataset=", "double_mnist", "nr_objs=", "clutter_static", "without_clutters", "grid_size=", "dumps", "filename="])
	for opt in opts:
		if opt[0] == "--batch_size":
...
def __step(img, prev_bbox, prev_att, state):
	...
	flat1 = TT.reshape(act1, (batch_size, conv1_output_dim))
        norms = TT.sqrt(TT.sqr(flat1).sum(axis=1))
	...
	return bbox, att, gru_h, mask * amplifier.dimshuffle(0, 'x', 'x'), norms
...
sc,_ = T.scan(__step, sequences=[imgs.dimshuffle(1, 0, 2, 3, 4)], outputs_info=[starts, startAtt, T.shared(NP.zeros((batch_size, gru_dim), dtype=T.config.floatX)), None, None])

bbox_seq = sc[0].dimshuffle(1, 0, 2)
att_seq = sc[1].dimshuffle(1, 0, 2)
mask_seq = sc[3].dimshuffle(1, 0, 2, 3)
norm_seq = sc[4].dimshuffle(1, 0)
...
test = T.function([seq_len_scalar, imgs, starts, startAtt, targets], [cost, bbox_seq, att_seq, mask_seq, norm_seq], allow_input_downcast=True)
import cPickle
...
bmnist = BouncingMNIST(nr_objs, seq_len, batch_size, img_row, dataset_name+"/inputs", dataset_name+"/targets", acc=acc_scale, scale_range=zoom_scale, clutter_move = clutter_move, with_clutters = with_clutters, buff=False, filename=filename)
try:
	_iou=[]
	for i in range(0, 1):
		for j in range(0, 1000):
			...
			#_len = int(RNG.exponential(seq_len - 5) + 5) if variadic_length else seq_len	
		        data, label = bmnist.GetBatch(count = 2 if double_mnist else 1)
                        if dumps:
                            NP.save(model_name + '-data-' + str(j) + '.npy', data)
                            NP.save(model_name + '-label-' + str(j) + '.npy', label)
			...
			att = np.zeros((batch_size, att_params))
                        ...
                        att[:, 1] = 1
                        att[:, 2] = 0
			cost, bbox_seq, att_seq, mask, norms = test(_len, data, label[:, 0, :], att, label)
			print 'Attention, sigma, strideH, strideW', att_seq
			...
			print NP.average(iou, axis=1)
                        if dumps:
                            NP.save(model_name + '-pred-' + str(j) + '.npy', (bbox_seq + 1) * (img_row / 2.))
                            NP.save(model_name + '-mask-' + str(j) + '.npy', mask)
                            #NP.save(model_name + '-vis-' + str(j) + '.npy', mask * data)
                        _iou.append(NP.average(iou, axis=1))
                        print 'Norms'
                        print norms
                        print 'Mean, Max, Min, Median, Std of IOU'
                        print NP.mean(_iou), NP.max(_iou), NP.min(_iou), NP.median(_iou), NP.std(_iou)
